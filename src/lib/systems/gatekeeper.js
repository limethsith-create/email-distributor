...
export async function runOnboardingNudge({ clientId, now = io.now() }) {
  ...
  await sendClient(clientId, 'onboarding_reminder', { firstName: firstNameOf(client.contactName), ownerName: await ownerName(clientId), link: pageUrl(token, 'onboard'), closeDate }, { dedupe: `onboarding_reminder:${due}` });

...
function AreaChart({ pts }) {
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} width="100%" style={{ display: 'block' }}>
      <defs>
        <linearGradient id="gUS" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor={C.us} stopOpacity="0.28" /><stop offset="100%" stopColor={C.us} stopOpacity="0.02" /></linearGradient>
        <linearGradient id="gLK" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor={C.lk} stopOpacity="0.24" /><stop offset="100%" stopColor={C.lk} stopOpacity="0.02" /></linearGradient>
      </defs>
  ...
  return (
    <div className="fade-up" style={{ maxWidth: 1180 }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 4 }}>
        ...
        <span style={{ display: 'inline-flex', alignItems: 'center', gap: 7, background: 'var(--accent-soft)', color: 'var(--accent)', border: '1px solid rgba(110,86,207,0.25)', borderRadius: 999, padding: '5px 12px', fontSize: 12.5, fontWeight: 600 }}>
          <span style={{ width: 7, height: 7, borderRadius: 999, background: 'var(--accent)', display: 'inline-block' }} /> Warmup mode · sending paused
        </span>
      </div>
      <p style={{ color: 'var(--fg-muted)', fontSize: 14, marginBottom: 20 }}>Your outreach at a glance.</p>

      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(200px,1fr))', gap: 14, marginBottom: 18 }}>
        <KPI label="Total leads" value={loading ? '—' : fresh.length} sub={`${us} US · ${lk} Sri Lanka`} accent="#6e56cf"
        ...
        <KPI label="Inboxes warming" value="2" sub="AutoMailer · active" accent="#16a34a"
        ...
        <KPI label="Emails sent" value="0" sub="paused for warmup" accent="#d97706"
        ...
        <KPI label="Replied" value="0" sub="campaigns not started" accent="#0891b2"
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(300px,1fr))', gap: 18 }}>
          ...
          {inboxes.map((b) => (
            <div key={b.email} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '11px 0', borderTop: '1px solid var(--border)' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexShrink: 0 }}>
                ...
                <span style={{ fontSize: 11.5, color: '#15803d', background: 'rgba(22,163,74,0.12)', borderRadius: 8, padding: '3px 9px' }}>Warming</span>
              </div>
            </div>
          ))}
        ...
        <div style={{ ...card, padding: '20px 22px' }}>
          ...
          {[
            { k: 'Ready to send (fresh)', v: fresh.length, c: '#6e56cf' },
